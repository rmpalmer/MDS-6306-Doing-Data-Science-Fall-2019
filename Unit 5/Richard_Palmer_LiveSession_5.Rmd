...
```{r}
BBall %>% drop_na(TotalInches) %>% ggplot(aes(x=TotalInches,fill=position)) + 
...
Make a plot over all positions, with a smooth trend line
```{r}
FIFA %>% drop_na(TotalInches) %>% drop_na(TotalPounds) %>% ggplot(aes(x=TotalInches,y=TotalPounds)) + geom_point() + geom_jitter(width=0.1,height=0.1) + geom_smooth() + ggtitle("Weight Versus Height for all positions, with smoothed trend line")
```

Now make separate scatter plots for each position
```{r}
FIFA %>% drop_na(TotalInches) %>% drop_na(TotalPounds) %>% ggplot(aes(x=TotalInches,y=TotalPounds)) + geom_point() + facet_wrap(~Position) + ggtitle("Weight versus height by position")
```

```{r}
FIFA %>% select(TotalInches,TotalPounds) %>% drop_na(TotalInches) %>% drop_na(TotalPounds) %>% ggpairs()
```

## limit to only LB and LM positions
```{r}
FIFA %>% filter(grepl("(LM|LB)",Position)) %>% drop_na(TotalInches) %>% drop_na(TotalPounds) %>%
  ggplot(aes(x=TotalInches,y=TotalPounds)) + geom_point() + geom_smooth() + facet_wrap(~Position) + ggtitle("Weight versus Height for LB and LM positions")
```
...
## Question 1
```{r}
bad <- df %>% filter(grepl("yyy$",name,ignore.case = TRUE))
as.character(bad$name)
dim(df)[1]
y2016 <- df %>% filter(!grepl("yyy$",name,ignore.case=TRUE))
dim(y2016)[1]
...
We also want to handle the combination of gender+name, rather than just name, so that for instance,
we handle the male name of Lee separately from the female name of Lee.
```{r}
...
does a histogram show significant detail in the distribution, and it is still a very skewed distribution.
...
Here is a basic analysis of that dataset.
...
This is a plot of the log of 

The bootstrap_p function create a p-value for testing the hypothesis that the data
came from a power distribution.  For the moby dick words, the p value is relatively
high, indicating that a power law distribution is an appropriate model.
```{r}
...
Now, to return to baby names, here is the same analysis.
```{r}
...
```{r}
raw <- round(100* rnorm(1000))
floor = min(raw)
my_normal <- raw - floor + 1
norm_pl <- displ$new(my_normal)
est <- estimate_xmin(norm_pl)
norm_pl$setXmin(est)
plot(norm_pl)
lines(norm_pl,col=2)
```