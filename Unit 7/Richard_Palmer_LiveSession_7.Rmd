...
titanicClean <- passenger_df %>% select(fields.age,
                                        fields.pclass,
                                        fields.survived,fields.sex) %>%
...
CM_nb = confusionMatrix(table(predict(model_b,testTitanic[,pred_indices]),
                              as.factor(testTitanic$Survived)))
CM_nb
```
...
```{r}
knn_pred_titanic <- knn(trainTitanic[,1:2],
                        testTitanic[,1:2],trainTitanic$Survived,k=3,prob=TRUE)
CM_knn_titanic   <- confusionMatrix(knn_pred_titanic,testTitanic$Survived)
CM_knn_titanic
```
## Run a few times to see variability in statistics
```{r}
iterations = 5
acc_nb = data.frame(nrow=iterations,ncol=4)
...
for (j in 1:iterations)
{
  ...
  CM_c <- confusionMatrix(table(predict(model_c,testTitanic[,pred_indices]),
                                as.factor(testTitanic$Survived)))
            
  acc_nb[j,1] <- j              
  acc_nb[j,2] <- CM_c$overall[1] # accuracy
  acc_nb[j,3] <- CM_c$byClass[1] # sensitivity
  acc_nb[j,4] <- CM_c$byClass[2] # specificity
}
colnames(acc_nb) <- c("Trial","Accuracy","Sensitivity","Specificity")
acc_nb %>% ggplot(aes(x=Trial)) + 
  geom_line(aes(y=Accuracy)) +
  geom_line(aes(y=Sensitivity)) +
  geom_line(aes(y=Specificity)) +
  xlab("Trial") + 
  ylab("Rate") + 
  ggtitle("Varying accuracy with different split trials")
```
...
From Naive Bayes using only Age and Pclass,
using a 70% train/test split and iterating 100 times, 
...
CM_d <- confusionMatrix(table(predict(model_d,
                                      testTitanic[,pred_indices]),
                                      as.factor(testTitanic$Survived)))
CM_d
```

## Iterate with sex in the model
```{r}
...
for (j in 1:iterations)
{
  ...
  CM_d <- confusionMatrix(table(predict(model_d,
                                        testTitanic[,pred_indices]),
                                        as.factor(testTitanic$Survived)))
  acc_nb_wsex[j,1] <- CM_d$overall[1] # accuracy
  acc_nb_wsex[j,2] <- CM_d$byClass[1] # sensitivity
  acc_nb_wsex[j,3] <- CM_d$byClass[2] # specificity
}
...
From Naive Bayes with Sex in the model,
using a 70% train/test split and iterating 100 times, 
...
masterAccNB = matrix(nrow = iterations,ncol=3)
...
for(j in 1:iterations)
{
  ...
  model = naiveBayes(train[,pred_indices],
                     as.factor(train$Species),laplace = 1)
  table(predict(model,test[,pred_indices]),
                as.factor(test$Species))
  CM = confusionMatrix(table(predict(model,
                                     test[,pred_indices]),
                                     as.factor(test$Species)))
  masterAccNB[j,1] = CM$overall[1]
  masterAccNB[j,2] = CM$byClass[1]
  masterAccNB[j,3] = CM$byClass[2]
}
MeanAccNB = colMeans(masterAccNB)

```
From Naive Bayes to predict multinomial species,
using a 70% train/test split and iterating 100 times, 

Mean Accuracy is `r MeanAccNB[1]`

Mean Sensitivity is `r MeanAccNB[2]`

Mean Specificity is `r MeanAccNB[3]`

## KNN for multinomial iris
```{r}
# from Unit 6, peak accuracy was with k = 33
my_k = 33
acc_knn = matrix(nrow=iterations,ncol=3)
...
for (j in 1:iterations)
{
  ...
  acc_knn[j,2] = cm$byClass[1]
  acc_knn[j,3] = cm$byClass[2]
}
MeanAccKnn = colMeans(acc_knn)

```

From KNN to predict multinomial species,
using a 70% train/test split and iterating 100 times, 

Mean Accuracy is `r MeanAccKnn[1]`

Mean Sensitivity is `r MeanAccKnn[2]`

Mean Specificity is `r MeanAccKnn[3]`
