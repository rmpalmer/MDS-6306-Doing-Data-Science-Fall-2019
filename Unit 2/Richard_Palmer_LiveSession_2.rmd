...
BBall <- raw_ball %>% rowwise() %>% mutate(htc=as.character(height),height_inches=to_inches(htc))
...
```{r,echo=TRUE}
p <- plot_ly(BBall, x=~height_inches,y=~weight,z=~year_start,color=~position) %>% add_markers %>% layout(scene=list(xaxis=list(title="Height"),yaxis=list(title="Weight"),zaxis=list(title="Start Year")))
```
