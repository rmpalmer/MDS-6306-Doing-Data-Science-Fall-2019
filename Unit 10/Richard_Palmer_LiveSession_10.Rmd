...
slope of `r ` if the null hypothesis is true.
...
slope_lo  <- slope_est - tvalue*slope_se
slope_hi  <- slope_est + tvalue*slope_se
```

slope est is `r slope_est`  
slope_se is  `r slope_se`  
slope_lo is `r slope_lo`  
slope_hi is `r slope_hi`  
A 95% confidence interval for the slope of the regression line is
[`r slope_lo` .. `r slope_hi`]
...
slope_est <- summary(model3)$coefficients[2,"Estimate"]
slope_se  <- summary(model3)$coefficients[2,"Std. Error"]
slope_lo  <- slope_est - tvalue*slope_se
slope_hi  <- slope_est + tvalue*slope_se
```
