...
```{r}
#fifa_all %>% rowwise() %>% mutate(value_factor = #cut(log_value,breaks=c(0,7.5,14,20),labels=c("low","mid","high"))) %>% #ggplot(aes(x=height_inches,y=weight_pounds)) + geom_point() + facet_wrap(value_factor) + #ggtitle("weight to height by value factor")
```
