...
```{r}
iris_cross_validate <- knn.cv(iris[,c(1,2)],iris$Species,k=90)
confusionMatrix(iris_cross_validate,iris$Species)
```
...
pottery <- raw %>% drop_na()
pottery
pot_cf <- confusionMatrix(table(knn.cv(pottery[,3:7],pottery$Site, k = 3), pottery$Site))
QOI = data.frame(Al = 21, Fe = 6.7, Mg = 4.9, Ca = 0.10, Na = 0.11)
knn(pottery[,3:7],QOI,pottery$Site, prob = TRUE, k = 3)
knn(pottery[,3:7],QOI,pottery$Site, prob = TRUE, k = 5)
```
